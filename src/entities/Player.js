...
export class Player extends Entity {
  constructor(x, y) {
    super(x, y, 16, 18);
    this.maxHp = 6;     // 3 hearts (2 per heart)
    this.hp = this.maxHp;
    this.hunger = 100;
    this.maxHunger = 100;
    this.stamina = 100;
  ...
  draw(ctx, off) {
    if (this.dead) {
      ...
      drawPlayer(ctx, this.x - off.x, this.y - off.y, this.dir, false, true);
      ctx.restore();
      return;
    }
    // i-frame blink
    if (this.invulnT > 0 && Math.floor(this.invulnT * 24) % 2 === 0) return;
    drawPlayer(ctx, this.x - off.x, this.y - off.y, this.dir, this.attackT > 0, this.hurtT > 0);
